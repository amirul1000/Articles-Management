...
class Articles extends CI_Controller{
    ...
    function index($start=0){
		$limit = 10;
        $data['articles'] = $this->Articles_model->get_limit_users_articles($limit,$start);
		//pagination
		$config['base_url'] = site_url('member/articles/index');
		$config['total_rows'] = $this->Articles_model->get_count_users_articles();
		$config['per_page'] = 10;
        ...
        $data['_view'] = 'member/articles/index';
...
$updated_at = "";
		...
		$params = array(
					 'author_users_id' => $this->session->userdata('id'),
...
'updated_at' =>$updated_at,
        ...
        if(isset($id) && $id>0){
            ...
            if(isset($_POST) && count($_POST) > 0){   
                ...
                redirect('member/articles/index');
            }else{
                $data['_view'] = 'member/articles/form';
                $this->load->view('layouts/member/body',$data);
            }
        } //save
		else{
	...
	function details($id){
        $data['articles'] = $this->Articles_model->get_articles($id);
		$data['id'] = $id;
        $data['_view'] = 'member/articles/details';
        $this->load->view('layouts/member/body',$data);
	}
...
$this->db->or_like('updated_at', $key, 'both');
		...
		//pagination
		$config['base_url'] = site_url('member/articles/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('author_users_id', $key, 'both');
$this->db->or_like('category_id', $key, 'both');
...
$this->db->or_like('updated_at', $key, 'both');
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/member/articles/print_template.php');
			$html = ob_get_clean();
